...
public class ServiceDetailsWindow extends TupaDetailsWindow {
	 ...
	 * @param caption
	 */
	public ServiceDetailsWindow(Translator translator, ExtPalvelu extPalvelu) {
		super(translator);
		addStyleName("service-details");
		this.extPalvelu = extPalvelu;
		init();
	}
	...
	private VerticalLayout getInfraDetails(ExtInfra extInfra) {
		...
		infraNimiLink.addClickListener(e -> {
			InfraDetailsWindow window = new InfraDetailsWindow(translator, TupaDBCache.getInfraCache().get(infra.getInfra_id()));
